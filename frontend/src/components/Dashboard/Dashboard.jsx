  ...
  return (
    <div className="flex flex-col items-center gap-2 w-full">
      <div className="w-full bg-[#F4F9FF] p-5 text-center">
        ...
        <p className="text-[#3B3838]">Manage your profile and see your contributions</p>
      </div>

      <div className="flex w-full max-w-7xl justify-center gap-4 lg:gap-8 mt-8 flex-col lg:flex-row">
        {/* Sidebar */}
        <div className="border-2 w-full lg:w-72 xl:w-80 flex flex-col items-center py-6 px-6 rounded-xl h-fit lg:h-[650px] bg-white shadow-[5px_7px_8px_rgba(0,0,0,0.25)]">
          <input type="file" ref={fileInputRef} onChange={handleAvatarChange} accept="image/*" hidden />

          <div className="relative group mb-4">
            <img src={profileData?.avatar_url || demo} alt="avatar" className={`w-24 h-24 lg:w-28 lg:h-28 rounded-full object-cover border-2 ${uploadingAvatar ? "opacity-50" : "opacity-100"}`} />
            {uploadingAvatar && (
          ...
          {avatarError && <p className="text-xs text-red-500 mb-2 text-center">{avatarError}</p>}

          <div className="text-center mb-6 w-full">
            <p className="font-bold text-lg lg:text-xl mb-2 break-words">
              {loading ? "Loading..." : error ? "Error" : `${profileData?.first_name || ""} ${profileData?.last_name || ""}`.trim() || "User Name"}
            </p>
            <p className="text-[#3B3838] text-sm lg:text-base break-words">
              {loading ? "..." : error ? "N/A" : profileData?.course?.name && profileData?.semester ? `${profileData.course.name} ${profileData.semester}th Semester` : "Course Info N/A"}
            </p>
          </div>

          {/* Profile Completion */}
          <div className="w-full mb-4">
            <div onClick={() => setIsModalOpen(true)} className="flex items-center justify-between gap-4 border rounded-lg px-4 py-3 text-sm cursor-pointer bg-white hover:bg-gray-50">
              <div className="flex items-center gap-2">
                <UserIcon className="w-5 h-5 text-gray-600" />
                <span className="text-gray-700 font-medium">Profile Completion</span>
              </div>
              <span className="text-[#C79745] font-semibold">{loading ? "..." : error ? "N/A" : `${profileCompletion}%`}</span>
            </div>
          </div>
        ...
        <div className="flex mb-10 flex-col border-2 rounded-xl w-full lg:flex-1 h-[650px] bg-white shadow-[5px_7px_8px_rgba(0,0,0,0.25)]">
          <div className="p-6 h-full overflow-y-auto">{active === "Contributions" ? <Contributions canUpload={canUpload} /> : active === "Rewards" ? <Rewards /> : <Downloads />}</div>
        </div>
      </div>
