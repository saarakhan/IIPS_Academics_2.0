...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      const { data: profile, error: profileError } = await supabase.from('profiles').select('*, course:course_id(name, duration_years), total_uploads, avatar_url').eq('id', userId).single();
  ...
  return (
    <div className='flex flex-col items-center gap-2 mt-8 w-full'>
      ...
      <div className='w-full max-w-7xl'>
        ...
        <p className='text-center text-[#3B3838] mt-1'>Manage your profile and see your contributions</p>
      </div>
            ...
            {menuItems.map(item => (
              <button
                ...
                className={`flex items-center gap-4 border-2 rounded-lg px-4 py-3 text-sm cursor-pointer transition-colors w-full ${
                  active === item.label ? 'bg-gray-100 shadow-[4px_4px_0px_0px_rgba(0,0,0,0.2)]' : 'bg-white hover:bg-gray-50'
                }`}>
